...
struct ucx_test_suite : public ::testing::Test
{
    ...
    {
        int num_ranks, rank;
        HWMALLOC_CHECK_MPI_RESULT(MPI_Comm_size(MPI_COMM_WORLD, &num_ranks));
        HWMALLOC_CHECK_MPI_RESULT(MPI_Comm_rank(MPI_COMM_WORLD, &rank));
        if (rank == 0) std::cout << "initializing ucx..." << std::endl;

        // read run-time context
        ucp_config_t* config_ptr;
        HWMALLOC_CHECK_UCX_RESULT(ucp_config_read(NULL, NULL, &config_ptr));
        ...
        //// initialize UCP
        HWMALLOC_CHECK_UCX_RESULT(ucp_init(&context_params, config_ptr, &ucp_context));
