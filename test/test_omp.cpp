...
#pragma omp barrier
		if(tid==thrid){
			...
			if(hwmalloc::numa().get_node(pointers[j].get()) != hwmalloc::numa().local_node()) {
			    EXPECT_TRUE(hwmalloc::numa().get_node(pointers[j].get()) == hwmalloc::numa().local_node());
			}
		    }
		    for(int j=0; j<NBUFFERS; j++){
			h.free(pointers[j]);
...
#pragma omp master
	{
	    printf("SPREAD, free :: %d theads per numa node, %d numa nodes\n", thr_per_node, nnodes);
	    EXPECT_TRUE((nthr==nnodes) || (nthr%nnodes==0));
	}
...
#pragma omp master
	{
	    ...
	    EXPECT_TRUE(n_registrations==(thr_per_node*NITER*nnodes*(NBUFFERS-1) + nnodes));
	}
    }
}
